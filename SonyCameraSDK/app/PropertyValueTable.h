...
struct PropertyValueTable
{
    ...
    PropertyValueEntry<std::uint16_t> exposure_value;
...
std::vector<std::uint16_t> parse_exposure_value(unsigned char const * buf, std::uint32_t nval);
...
text format_exposure_value(std::uint16_t exposure_value);
