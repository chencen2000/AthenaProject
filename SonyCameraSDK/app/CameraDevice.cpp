...
void CameraDevice::get_exposure_value()
{
    load_properties();
    tout << "Exposure Value: " << format_exposure_value(m_prop.exposure_value.current) << '\n';
}
...
void CameraDevice::set_exposure_value()
{
    if (!m_prop.exposure_value.writable) {
        // Not a settable property
        tout << "Exposure Program Mode is not writable\n";
        return;
    }
    ...
    text input;
    tout << "Would you like to set a new Exposure Value? (y/n): ";
    std::getline(tin, input);
    if (input != TEXT("y")) {
        tout << "Skip setting a new value.\n";
        return;
    ...
    tout << "Choose a number set a new Exposure Value:\n";
    tout << "[-1] Cancel input\n";

    auto& values = m_prop.exposure_value.possible;
    for (std::size_t i = 0; i < values.size(); ++i) {
        tout << '[' << i << "] " << format_exposure_value(values[i]) << '\n';
    }
...
void CameraDevice::OnConnected(SDK::DeviceConnectionVersioin version)
{
    ...
    text id(this->get_id());
    tout << "Connected to " << m_info->GetModel() << " (" << id.data() << ")" << std::endl;
}
...
{
    text file(filename);
    tout << "Complete download. File: " << file.data() << std::endl;
}
    ...
    if (prop_list && nprop > 0) {
        ...
        for (std::int32_t i = 0; i < nprop; ++i) {
            ...
            case SDK::CrDevicePropertyCode::CrDeviceProperty_ExposureBiasCompensation:
                nval = prop.GetValueSize() / sizeof(std::uint16_t);
                m_prop.exposure_value.writable = prop.IsSetEnableCurrentValue();
                m_prop.exposure_value.current = static_cast<std::uint16_t>(prop.GetCurrentValue());
                if (nval != m_prop.exposure_value.possible.size()) {
                    auto parsed_values = parse_exposure_value(prop.GetValues(), nval);
                    m_prop.exposure_value.possible.swap(parsed_values);
                }
                break;
